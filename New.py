...
pygame.mixer.music.set_volume(0.5)
    ...
    # Pour le son du saut
jump_sound = pygame.mixer.Sound("Tower Of Heights/Saut.wav")#son très moche qui va changer
...
# --- Images et classes---
    # Heros
perso1_image = pygame.image.load("Tower Of Heights/archer-attaque.png").convert_alpha()
perso2_image = pygame.image.load("Tower Of Heights/épéiste_couleur.png").convert_alpha()

    # Fleche
arrow_img = pygame.image.load("Tower Of Heights/fleche.png").convert_alpha()
...
class Arrow:
    ...
    # Monstre
monster_img = pygame.transform.scale(pygame.image.load("Tower Of Heights/slug.png").convert_alpha(), (150, 112.5))
...
def menu_de_debut():
    global selected_image, selected_image_left, selected_image_right, selected_attack, selected_attack_left, selected_attack_right, perso_rect, state, player
    ...
    if perso1_rect_menu.collidepoint(event.pos):
        ...
        selected_attack = pygame.image.load("Tower Of Heights/archer_post_attaque.png").convert_alpha()
        ...
        if perso_rect.colliderect(monster.rect):
            ...
            if attack:
                if selected_image == selected_attack_left and player == "swordsman":
                    if monster.rect.x < perso_rect.x:  # Si le monstre est à gauche du joueur
                        monster.life -= 1              # Il perd une pygame.freetype.vertical
                        monster.rect.x += PUSHBACK     # Il recule
                elif selected_image == selected_attack_right and player == "swordsman":
                    if monster.rect.x > perso_rect.x:  # Si le monstre est à droite du joueur
