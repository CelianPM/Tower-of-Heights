...
def use_inventory_slot(inventory, slot_index, life, max_life, player_speed, puissance):
    """Utilise le slot demandé si utilisable et retourne (life, message)."""
    slot = inventory[slot_index]
    if slot is None:
        return life, player_speed, puissance, f"Slot {slot_index + 1} vide"

    if not slot.get("usable", False):
        return life, player_speed, puissance, f"{slot['name']} non utilisable"
    ...
    if item_name == "Potion_vie":
        if life < max_life:
            ...
            return life, player_speed, puissance, "Vie déjà au maximum"
    elif item_name == "Potion_vitesse":
        player_speed += slot.get("heal_amount", 0)
        used = True
    elif item_name == "Potion_puissance":
        puissance += slot.get("heal_amount", 0)
        used = True
    ...
    if used:
        ...
        return life, player_speed, puissance, f"{slot['name']} utilisé"

    return life, player_speed, puissance, "Aucun effet"
...
def game(start_time, direction, attack, on_ground, velocity, max_life, state, selected_image, selected_image_left, selected_image_right, selected_attack_left, selected_attack_right, hitbox, player, monsters, arrows, GRAVITY, jump_power, player_speed, PUSHBACK, camera_y, HEIGHT, time, key, last_attack_time, last_damage_time, attack_delay, attack_animation_time, can_attack, xp, level, point_attribut, life, regenaration_time, degat, puissance, items, inventory, slot_hold_start, slot_use_lock, last_inventory_feedback, last_inventory_feedback_time):
    ...
    for slot_index, key_code in enumerate(slot_keys):
        ...
        if key[key_code] and slot is not None and slot.get("usable", False):
            ...
            if not slot_use_lock[slot_index] and (time - slot_hold_start[slot_index] >= ITEM_USE_HOLD_MS):
                life, player_speed, puissance, feedback = use_inventory_slot(inventory, slot_index, life, max_life, player_speed, puissance)
    ...
    # Jeter un objet: maintenir A + numéro du slot (1..5)
    if key[pygame.K_a]:
        for slot_index, key_code in enumerate(slot_keys):
            if key[key_code] and not slot_use_lock[slot_index]:
                dropped_x = hitbox.centerx
                dropped_y = hitbox.bottom
...
def menu_attribut(screen, text_font, WIDTH, HEIGHT, RED, level, continue_rect, speed_rect, vitality_rect, puissance_rect, attack_delay_rect, player_speed, point_attribut, max_life, regenaration_time, attack_delay, puissance):
...
while running:
    ...
    if state == "menu_attribut":
        menu_attribut(screen, text_font, WIDTH, HEIGHT, RED, level, continue_rect, speed_rect, vitality_rect, puissance_rect, attack_delay_rect, player_speed, point_attribut, max_life, regenaration_time, attack_delay, puissance)
        continue
