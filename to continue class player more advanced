...
# --- Images et classes---
    # Heros
perso1_image = pygame.image.load("archer_attack.png").convert_alpha()   # Charger l'image de l'archer
perso2_image = pygame.image.load("epeiste_couleur.png").convert_alpha()  # Charger l'image de l'épéiste
perso3_image = pygame.image.load("ninja_ash.png").convert_alpha()
...
perso3_rect_menu = perso3_image.get_rect(center = (WIDTH//2 + 300, HEIGHT//2))
...
class Player:
    ...
    def select_the_image(self):
        ...
        if self.hero == "archer":
            ...
            self.walk_frames_right = [
                pygame.image.load("archer_attack.png").convert_alpha(),
            ]
            ...
            self.walk_frames_right = [
                ...
                pygame.image.load("epeiste_walk1.png").convert_alpha(),
                pygame.image.load("epeiste_walk2.png").convert_alpha(),
            ]
            self.walk_frames_left = [pygame.transform.flip(frame, True, False) for frame in self.walk_frames_right] 
            ...
            self.selected_image_right = self.selected_image                                                        # Profil droit de l'image sélectionnée
            self.selected_image_left = pygame.transform.flip(self.selected_image, True, False)                     # Profil gauche de l'image sélectionnée
            self.selected_attack = pygame.image.load("ninja_ash.png").convert_alpha()                   # Télécharge l'image de l'attaque de l'épéiste
            self.speed = 5
            self.max_life = 3
            self.regeneration_time = 20000
            self.degat = 300
            self.walk_frames_right = [
                pygame.image.load("ninja_ash.png").convert_alpha(),
            ]
            self.walk_frames_left = [pygame.transform.flip(frame, True, False) for frame in self.walk_frames_right]
    ...
    def move(self, jump_sound, state, time, key, velocity, start_time):
        ...
        if key[pygame.K_LEFT]:                                   # Si la touche de gauche est appuyée
            ...
            self.selected_image = self.animate(self.walk_frames_left)   # Si la direction précédente était à droite, changer l'image sélectionnée par celle du profil gauche
        ...
        if key[pygame.K_RIGHT]:                                  # Si la touche de droite est appuyée
            ...
            self.selected_image = self.animate(self.walk_frames_right)   # Si la direction précédente était à gauche, changer l'image sélectionnée par celle du profil droit
    ...
    def platform_collisions(self, platforms, velocity):
        # On recalcule l'état "au sol" à chaque frame pour tomber correctement en sortant d'un bord.
        self.on_ground = False

        for platform in platforms:
            horizontal_overlap = self.hitbox.right > platform.left and self.hitbox.left < platform.right

            # Collision verticale descendante (atterrissage), y compris si on traverse un tile en une frame.
            ...
            # Collision verticale montante (plafond).
            if velocity < 0 and horizontal_overlap:
            ...
            # Collisions latérales uniquement en cas de recouvrement réel.
    ...
    def monster_collisions(self, monsters, time, arrows):
        for monster in monsters[:]:
            if monster.alive and self.hitbox.colliderect(monster.rect):                           # Si le monstre est vivant et que sa hitbox est en collision avec celle du joueur
                ...
                if self.attack:
                    if self.selected_image == self.selected_attack_left and self.hero == "swordsman" or self.hero == "ninja":     # Si le joueur attaque vers la gauche avec l'épée
                    ...
                    elif self.selected_image == self.selected_attack_right and self.hero == "swordsman" or self.hero == "ninja":  # Si le joueur attaque vers la droite avec l'épée
            ...
            for shuri in shuris[:]:
                if monster.alive and shuri.rect.colliderect(monster.rect):                   # Si la hitbox de la flèche est en collision avec celle du monstre
                    monster.life -= self.degat + self.puissance                                                        # Le monstre perd une vie
                    if shuri.direction == "right":
                        monster.rect.x += PUSHBACK                                           # Si la flèche va vers la droite, le monstre recule vers la droite
                    else:
                        monster.rect.x -= PUSHBACK                                           # Si la flèche va vers la gauche, le monstre recule vers la gauche
                    shuri.remove(arrow)                                                     # Retirer la flèche du jeu
                    if monster.life <= 0:
                        monster.alive = False                                                # Quand le monstre n'a plus de vies, il est retiré du jeu
                        self.xp += monster.xp_reward
...
shuri_img = pygame.image.load("shuri.png").convert_alpha()  # Charger l'image du shuriken
...
def menu_de_debut(state, perso1_rect_menu, perso2_rect_menu, perso3_rect_menu, event, player):
    """Se charge de gérer les clics sur les personnages dans le menu de départ, et de définir les variables correspondantes en fonction du personnage choisi"""
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if state == "menu_de_debut" and event.type == pygame.MOUSEBUTTONDOWN:
            state, player = menu_de_debut(state, perso1_rect_menu, perso2_rect_menu, perso3_rect_menu, event, player)  # Appeler la fonction menu_de_debut() pour gérer les interactions avec les personnages sur la page du menu de départ, et récupérer les variables mises à jour par cette fonction
